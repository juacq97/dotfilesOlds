...
#+end_src
** Load fonts
Remember that part when the fonts function was declared. Well, now it's the time to load that function, and to end the =use-package= macro. This is ugly, though, I need to think a way to reduce all this writing.
#+begin_src emacs-lisp
  (my/org-font-setup)
